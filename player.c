...
void corrigePersonagemDireita(PLAYER *p) {
    (*p).posX -=1;
}

void corrigePersonagemEsquerda(PLAYER *p) {
    (*p).posX +=1;
}
...
void centralizaPlayerNaEscada(PLAYER *p, char mapa[TILES][TILES]){
    ...
    if (mapa[posYGrid][posXGrid]=='S' || mapa[posYGrid][posXGrid]=='H' || mapa[posYGrid][posXGrid]=='D'){
        (*p).velX=0; (*p).velY=0; // "Locka" o player na escada
        (*p).posX/=COMP_LINHA;
        (*p).posX*=COMP_LINHA;
    }
...
void processaPuloPlayer(PLAYER *p, char mapa[TILES][TILES]) {
    if (IsKeyPressed(KEY_SPACE)){
        ...
        if (playerNoChao(*p, mapa) || playerNaPlataforma(*p, mapa)){
            (*p).posY -= 5; // Tira o player do chao para que nao fique preso
            (*p).velY = P_VEL_PULO;
        }
    }
}
...
void processaColisoesPlayer(PLAYER *p, char mapa[TILES][TILES]) {
    ...
    if (colidiuBordaDireita(*p)) {
        corrigePersonagemDireita(p);
        (*p).velX=0;
    }
    if (colidiuBordaEsquerda(*p)) {
        corrigePersonagemEsquerda(p);
        (*p).velX=0;
    }
}
